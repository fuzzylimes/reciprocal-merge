...
export class cscash extends Base {
  ...
  async build() {
    ...
    const cscash = getCellValue(this.report, rs.summary, 'C14');

    if (Number(cscash) * 100 < 20) {
      // check values in rows 16-26 to see if Col C >= 20, include A and C if so
      const vals = [];
      for (let i = 16; i <= 26; i++) {
        const v = Number(getCellValue(this.report, rs.summary, `C${i}`)) * 100;
        if (v < 20) continue;

        const rawLabel = (getCellValue(this.report, rs.summary, `A${i}`));
